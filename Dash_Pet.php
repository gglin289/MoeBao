<?php $Page_Name="每日紀錄" ; $Page_Permission = 1 ; include("Assets/Header.php") ; 
    $Fetch = $Connection->query("SELECT * , date(`Time`) as 'Time', Service_Record.System_ID as 'ID' FROM `Service_Record` LEFT JOIN `Pet_Information` on Service_Record.Pet_ID = Pet_Information.System_ID WHERE `Time` LIKE '{$_GET['Date']}%' AND `Type` LIKE '美容' ORDER BY `Time` DESC")->fetchAll(PDO::FETCH_ASSOC) ;
...
<div class="table-responsive">
    <table id="List" class="table table-hover table-sm table-striped">
        ...
        <tbody>
            ...
            <tr>
                <td nowrap><?php echo $Val['Time'] ; ?></td>
